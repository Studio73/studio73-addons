...
class AccountSpecificIntracommunityTransactions(models.Model):
    ...
    operation_type = fields.Selection(
        ...
        selection=[('A', 'A - The transmission or receipt of goods to undertake partial reports or works stipulated in '
                   ...
                   ('B', 'B - Transfers of goods or intra-Community acquisitions of goods listed in Article '
                         '9.3 and Article 16.2 of the Tax Law (Law 37/1992).')],
        required=True
    )
    declared_key = fields.Selection(string='Declared key', selection=[('D', 'Filer'), ('R', 'Sender')], required=True)
    ...
    company_id = fields.Many2one(
        comodel_name='res.company', string='Company', required=True, default=_get_default_company
    )
    state = fields.Selection(
        string="State",
        selection=[("draft", "Draft"), ("validated", "Validated"), ("cancel", "Canceled")],
        default="draft"
    )
    ...
    sii_state = fields.Selection(
        ...
        help="Indicates the state of this good in relation with the presentation at the SII",
    )
    ...
    sii_send_failed = fields.Boolean(
        ...
        copy=False,
        help="Indicates that the last attempt to communicate "
             "this good to the SII has failed. See SII return for details",
    )
    ...
    def unlink(self):
        if not self._cancel_transaction_jobs():
            raise exceptions.Warning(_('You can not delete this transaction because there is a job running!'))
    ...
    def cancel_transactions(self):
        ...
        transactions = self.filtered(
            lambda t:
            (t.sii_enabled and t.state in ['cancel'] and t.sii_state in ['sent', 'sent_w_errors', 'sent_modified'])
        )
        ...
        for transaction in transactions:
            ...
            if not company.use_connector:
                ...
                new_delay = transaction.with_context(ctx).with_delay(
                    eta=eta if not transaction.sii_send_failed else False,
                ).confirm_one_transaction()
                queue_ids = queue_obj.search([('uuid', '=', new_delay.uuid)], limit=1)
                transaction.sudo().transaction_jobs_ids |= queue_ids

    @api.multi
    def to_draft_transactions(self):
        if not self._cancel_transaction_jobs():
            raise exceptions.Warning(_('You can not set to draft this transaction because there is a job running!'))
        self.state = 'draft'
    ...
    def _connect_sii(self, wsdl):
        ...
        if sii_config:
            ...
            public_crt = self.env['ir.config_parameter'].get_param('l10n_es_aeat_sii.publicCrt', False)
            private_key = self.env['ir.config_parameter'].get_param('l10n_es_aeat_sii.privateKey', False)
        session = Session()
    ...
    def _get_sii_transaction_dict(self, cancel=False):
        ...
        if not cancel:
            transaction_dict["Contraparte"] = {
                "NombreRazon": self.counterparty_name
            }
            if self.counterparty_identification_type == '1':
                transaction_dict["Contraparte"].update({
                    'NIF': self.counterparty_vat[2:]
                })
            ...
            if self.counterparty_representative_vat:
                transaction_dict["Contraparte"].update({
                    'NIFRepresentante': self.counterparty_representative_vat[2:]
                })
            if self.operation_term:
                transaction_dict["OperacionIntracomunitaria"].update({
                    'PlazoOperacion': self.operation_term
                })
            if self.other_inv_doc:
                transaction_dict["OperacionIntracomunitaria"].update({
                    'FacturasODocumentacion': self.other_inv_doc
                })
    ...
    def _get_sii_header(self, tipo_comunicacion=False, cancellation=False):
        ...
        if not company.vat:
            raise exceptions.Warning(_("No VAT configured for the company '{}'").format(company.name))
        header = {
        ...
        if not cancellation:
            header.update(dict(TipoComunicacion=tipo_comunicacion))
        return header
    ...
    @job
    @api.multi
    def confirm_one_transaction(self):
        self._send_transaction_to_sii()

    @job
    @api.multi
    def cancel_one_transaction(self):
        self._cancel_transaction_to_sii()