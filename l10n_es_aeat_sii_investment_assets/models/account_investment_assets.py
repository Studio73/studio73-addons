...
class AccountInvestmentAssets(models.Model):
    ...
    sii_state = fields.Selection(
        selection=SII_STATES, string="SII send state", default='not_sent', readonly=True, copy=False,
    ...
    def unlink(self):
        if not self._cancel_asset_jobs():
            raise exceptions.Warning(_('You can not delete this asset because there is a job running!'))
    ...
    def cancel_assets(self):
        if not self._cancel_asset_jobs():
            raise exceptions.Warning(_('You can not cancel this asset because there is a job running!'))
        ...
        assets = self.filtered(
            lambda a:
            (a.sii_enabled and a.state in ['cancel'] and a.sii_state in ['sent', 'sent_w_errors', 'sent_modified'])
        )
        ...
        for asset in assets:
            ...
            if not company.use_connector:
                ...
                new_delay = asset.with_context(ctx).with_delay(
    ...
    def to_draft_assets(self):
        if not self._cancel_asset_jobs():
            raise exceptions.Warning(_(
                'You can not set to draft this asset because there is a job running!'))
        self.state = 'draft'
    ...
    def _send_asset_to_sii(self):
        for asset in self.filtered(lambda a: a.state in ['validated']):
            company = asset.company_id
            wsdl = self.env['ir.config_parameter'].get_param('l10n_es_aeat_sii.wsdl_pi', False)
    ...
    @job
    @api.multi
    def confirm_one_asset(self):
        self._send_asset_to_sii()

    @job
    @api.multi
    def cancel_one_asset(self):
