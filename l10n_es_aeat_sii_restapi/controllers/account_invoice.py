...
LINE_RE_TYPES = [0, 0.5, 1.4, 1.75, 5.2]
...
class AccountInvoiceController(Controller):
    ...
    def account_invoice_create(self, **kwargs):
        ...
        country = request.env['res.country'].search([('code', '=', partner['country_code'])], limit=1)
        if not country:
        ...
        if kwargs['type'] not in ['out_invoice', 'out_refund', 'in_invoice', 'in_refund']:
        ...
        registration_key = request.env['aeat.sii.mapping.registration.keys'].search(
        ...
        if vals.get("period", False) and vals.get("year", False):
            period_id = request.env['account.period'].search([('code', '=', '%s/%s' % (vals["period"], vals["year"]))])
        if not period_id:
        ...
        if kwargs.get("operation", 'A0') == 'A0':
            try:
                inv_exists = request.env['account.invoice.import'].search([('number', '=', vals['number'])])
                if inv_exists:
                    return self._error(4308, _('Duplicated invoice number, use A1 operation to substitute it'))
                ...
                print str(e)
                return self._error(
                    5101, _('Company is not available to receive invoices. Contact with the IT support team')
                )
        elif kwargs["operation"] == 'A1':
            invoice_import = request.env['account.invoice.import'].search([('number', '=', vals['number'])])
            if not invoice_import:
                return self._error(
                    4307, _('The invoice number does not exist, use A0 operation to create it')
                )
