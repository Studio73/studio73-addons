...
class TestAccountInvoice(HttpCase):
    ...
    def setUp(self):
        ...
        req = requests.post(
            ...
            data=json.dumps({'params': {'db': 'SII', 'login': USER, 'password': PSWD}})
        )
        self.cookies = {'session_id': json.loads(req.text)['result']['session_id']}
        self.invoice_ok = {
            'partner': {
    ...
    def get_post_request(self, invoice_data):
        req = requests.post(
            self.url,
            headers={'Content-type': 'application/json'},
            data=json.dumps({'params': invoice_data}),
